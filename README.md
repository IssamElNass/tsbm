...
2. *(Optional)* **Pre-seed** — if `ingestion_seed_rows > 0`, inserts that many rows so measurement rounds append into a pre-populated table.
...
| `tag_cardinality_threshold`| float     | `0.05`                              | A string column is classified as a *tag* (low-cardinality, indexed) if the ratio of unique values to total rows is ≤ this value. Above the threshold it is treated as a plain `TEXT` metric. |
...
export TSBM_DATABASES__QUESTDB__HOST=192.168.1.10
